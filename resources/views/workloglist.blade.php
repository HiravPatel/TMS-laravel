            ...
            <div class="table-responsive">
                <table class="table align-middle table-hover">
                    ...
                    <tbody>
                        @forelse ($logs as $log)
                            <tr>
                                <td><span class="badge bg-warning-subtle text-dark px-3 py-2">{{ $log->project->name }} - {{ $log->project->id }}</td>
                                <td>{{ $log->user->name }}</td>
                                <td>{{ $log->description }}</td>
                                <td><span class="badge bg-subtle text-light px-3 py-2" style="background-color: #68668e">{{ \Carbon\Carbon::parse($log->date)->format('d M, Y') }}</span></td>
                            </tr>
                        @empty
                            <tr>
